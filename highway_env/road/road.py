...
class Road(object):
    ...
    def __init__(self,
                 ...
                 record_history: bool = False,ints=None) -> None:
        """
        New road.
        ...
        '''
        in paper:
           water depth: 10,20,30cm
           intensity  : 4,8,12 mm/h

        in the code:
            water depth= U(0,30)
            intensity = U(0,12)
        '''
        ## init road rainfall conditions (intensities mm/hr)
        if not ints:
            levels=[2,4,6,8,10,12]
            self.intensity=choices(levels,k=1)[0] ## rainfall intensity
        else:
            self.intensity=ints
            
        self.depth= self.computeDepth()  # in mm
    ...
    def computeDepth(self):
        ...
        ints = self.intensity/60 ## convert to mm/min
    ...
    def act(self) -> None:
        ...
        for vehicle in self.vehicles:
            # if type(vehicle)==highway_env.vehicle.behavior.KraussVehicle:
            #     vehicle.act(depth=self.depth,intensity=self.intensity)
            # else:
            vehicle.act()
    ...
    def neighbour_vehicles(self, vehicle: 'kinematics.Vehicle', lane_index: LaneIndex = None,capped=None) \
        ...
        for v in self.vehicles + self.objects:
            if v is not vehicle and not isinstance(v, Landmark):  # self.network.is_connected_road(v.lane_index,
                ...
                if s_v < s and (s_rear is None or s_v > s_rear):
                    ...
                    v_rear = v
        if capped:
            s_front= s_front if s_front else capped+1
            s_rear= s_rear if s_rear else capped+1

            v_front=None if s_front>capped else s_front
            v_rear=None if s_rear>capped else s_rear
