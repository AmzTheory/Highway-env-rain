...
class OccupancyGridObservationExt(OccupancyGridObservation):
    def __init__(self, env: 'AbstractEnv', **kwargs) -> None:
    ...
    def space(self) -> spaces.Space:
        if self.as_image:
            ...
            return spaces.Dict({
                        'snap':spaces.Box(low=-np.inf, high=np.inf,shape=(3,self.grid.shape[1],self.grid.shape[2]),dtype=int),
                        'speed':spaces.Box(low=-np.inf, high=np.inf,shape=(1,3)),
                        ## intens, , exp_speed
                        'info' :spaces.Box(low=-np.inf, high=np.inf,shape=(1,2))
                        })
    ...
    def pos_to_index(self, position: Vector, relative: bool = False) -> Tuple[int, int]:
        ...
        if self.align_to_vehicle_axes:
            c, s = np.cos(self.observer_vehicle.heading), np.sin(self.observer_vehicle.heading)
            position = np.array([[c, s], [-s, c]]) @ position
        a,b= int(np.floor((position[0] - self.grid_size[0, 0]) / self.grid_step[0])), int(np.floor((position[1] - self.grid_size[1, 0]) / self.grid_step[1]))

         
        return a+3,b
        # return a,b

    def observeM(self) -> np.ndarray:
        if not self.env.road:
            return np.zeros(self.space().shape)
        ...
        if self.absolute:
            ...
            for layer, feature in enumerate(self.features):
                if feature in df.columns:  # A vehicle feature
                    for _, vehicle in df.iterrows():
                        x, y,w,l = vehicle["x"], vehicle["y"], vehicle["wid"],vehicle['len']
                        # Recover unnormalized coordinates for cell index
                        if "x" in self.features_range:
                            x = utils.lmap(x, [-1, 1], [self.features_range["x"][0], self.features_range["x"][1]])
                        if "y" in self.features_range:
            ...
            obs = np.nan_to_num(obs).astype(super().space().dtype)

            
            return obs

    def MinMaxScale(self,x,min,max):
    ...
    def observe(self):
        ...
        vel=None
        M=self.observeM()
        obs_veh=self.observer_vehicle

        
        sc_speed= self.MinMaxScale(obs_veh.target_speed,8.3,16.6)
        ints=self.MinMaxScale(obs_veh.intensity,2,12)
        info=[ints,sc_speed]
        ##         mm/h              m/s

        ## When being in the far left spot, cells to your left will be all one
        ## similarily when being far right spot
        _from, _to, _id = obs_veh.target_lane_index
        if _id==0 or _id==2:
        ...
        ##obs is composed M,M-1,M-2,V,V-1,V-2,info

        V=self.MinMaxScale(obs_veh.speed-obs_veh.target_speed,-8.3,0)

        # spConc=(np.array([[dv]]), self.state["speed"][:1:])
        spConc=(obs_veh.V[:,1:],np.array([[V]]))
        speed=np.concatenate(spConc,axis=1)
        obs_veh.V=speed
        
        
        snConc=(obs_veh.M[1:],np.array(M))
        snap=np.concatenate(snConc,axis=0)
        obs_veh.M=snap
    ...
    def identifyPos(self,position,l,w):

        ## neglect in case its the controlled vehicle
        if position[0]==0 and position[1]==0:
            return None,(0,0)

        # Avoid considering rear and front (not important for the LC)

        ## tol??
        tol=1.2

        #Left Back
        if position[0]<=0 and position[1]<-tol:
            if abs(position[0])<l:#3 incase vehicles overlap along the x
                return 0,(0,0)
            return 0,(position[0]-l,position[1]-w)
        ## Right Back
        elif position[0]<=0 and position[1]>tol:
            if abs(position[0])<l:
                return 1,(0,0)
            return 1,(position[0]-l,position[1]-w)
        ## Left Front
        elif position[0]>=0 and position[1]<-tol:
            if abs(position[0])<l:
                return 2,(0,0)
            return 2,(position[0]-l,position[1]-w)
        ## right Front
        elif position[0]>=0 and position[1]>tol:
            if abs(position[0])<l:
                return 3,(0,0)
            return 3,(position[0]-l,position[1]-w)

        return None,(0,0)
    def computeRad(self,position):
...
def observation_factory(env: 'AbstractEnv', config: dict) -> ObservationType:
    ...
    elif config["type"] == "OccupancyGridExt":
        return OccupancyGridObservationExt(env, **config)
    else:
