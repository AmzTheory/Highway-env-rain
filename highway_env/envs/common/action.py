...
from highway_env.vehicle.controller import HamVehicle, MDPVehicle
...
class DiscreteMetaAction(ActionType):
    ...
    def __init__(self,
                 ...
                 longitudinal: bool = True,
                 lateral: bool = True,
                 veh_class: str="highway_env.vehicle.controller.MDPVehicle",
                 **kwargs) -> None:
        """
        ...
        self.veh_class=utils.class_from_path(veh_class)
    ...
    def act(self, action: int) -> None:
        if type(self.controlled_vehicle)==HamVehicle:
            self.controlled_vehicle.isLC=True if action==0 or action==2 else False
